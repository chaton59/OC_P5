...
def load_raw_data(sondage_path='../raw_data/extrait_sondage.csv',
    ...
    eval_df['augementation_salaire_precedente'] = eval_df['augementation_salaire_precedente'].apply(
    ...
    central_df = pd.merge(central_df, sirh, left_on='employee_id', right_on='id_employee', how='inner')
    central_df.drop(['code_sondage', 'eval_number', 'id_employee', 'employee_id'], axis=1, inplace=True, errors='ignore')
    return central_df
...
def preprocess_data(raw_data_paths=None):
    ...
    columns_to_drop = ['ayant_enfants'] if len(central_df['ayant_enfants'].unique()) == 1 else []  # Constante
    ...
    for col in quantitative_cols:
        if central_df[col].std() > 0 and np.sum(np.abs(stats.zscore(central_df[col])) > 3) > 0:
            central_df[col] = winsorize(central_df[col], limits=[0.01, 0.01])
    
    # Engineering (comme improvement.py : ratios, moyennes ; +1 évite div0)
    central_df['revenu_par_anciennete'] = central_df['revenu_mensuel'] / (central_df['annees_dans_l_entreprise'] + 1)
    central_df['experience_par_anciennete'] = central_df['annee_experience_totale'] / (central_df['annees_dans_l_entreprise'] + 1)
    central_df['satisfaction_moyenne'] = central_df[['satisfaction_employee_environnement', 'satisfaction_employee_nature_travail',
    ...
    central_df['promo_par_anciennete'] = central_df['annees_depuis_la_derniere_promotion'] / (central_df['annees_dans_l_entreprise'] + 1)
    ...
    encoded_non_ord = pd.DataFrame(onehot.fit_transform(central_df[cat_non_ord]),
    ...
    encoded_ord = pd.DataFrame(ordinal.fit_transform(central_df[cat_ord]), columns=cat_ord)
    
    # Assemblage
    df_engineered = pd.concat([central_df[quantitative_cols], encoded_non_ord, encoded_ord,
                               central_df['a_quitte_l_entreprise']], axis=1)  # Inclut cible
    
    # Scaling (quantitatives + ordinal)
    cols_to_scale = quantitative_cols.tolist() + cat_ord + ['revenu_par_anciennete', 'experience_par_anciennete',
                                                            'satisfaction_moyenne', 'promo_par_anciennete']
